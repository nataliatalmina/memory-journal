...
- **Privacy policy:** `Onboarding/PrivacyPolicyView.swift` is in-app and scrollable. The wording is now **finalised** (effective 19 June 2026) and written to match actual behaviour; keep it in sync if data handling ever changes. Contact address in the policy is **hello@keepsakejournal.app**. NOTE: the App Store also needs a **publicly hosted copy at a URL** (keepsakejournal.app/privacy) — same text, including the same contact address — entered in App Store Connect, plus a reachable Support URL (keepsakejournal.app/support). The old `memoryjournalapp.com` domain was replaced when the app was renamed to keepsake.
...
- **The app must not request photo-library access at all.** `PhotosPicker` runs out-of-process: the picker UI is drawn by a separate system process, and the app receives only the images the user selected. It needs no authorization, and the old `PHPhotoLibrary` `.readWrite` request was for access the app never used. `MediaCapability` has **no `photoLibrary` case**, `MediaPermissions` no longer imports `Photos`, and `INFOPLIST_KEY_NSPhotoLibraryUsageDescription` was **removed** from both build configs. Only add it back if the app ever genuinely writes to the user's library (that would need `.addOnly`, requested at the moment the user taps save).
...
**Scheduled change — photo-library access returns in Phase 7.** The "must not request photo-library access at all" rule above **still stands until Phase 7 ships**; don't pre-emptively add the permission back. Phase 7 (photo look-back — filling empty look-back slots with a photo taken on the same date) genuinely reads the library by capture date, which `PhotosPicker` cannot do, so it reinstates `NSPhotoLibraryUsageDescription` and a `.readWrite` request — this time for access the app actually uses, which is what 5.1.1(iv) asks for. The other 5.1.1(iv) rules are unaffected and the plan is built around them: the onboarding toggle stores intent only and calls no Photos API, and the system prompt is triggered by the user tapping an in-context card on the Journal screen, at the point of use. **Full plan: [`docs/photo-lookback-plan.md`](../docs/photo-lookback-plan.md)** — read it before starting Phase 7, and update this bullet to a SUPERSEDED entry when it lands.
...
- [ ] **Phase 7 — Photo look-back** (opt-in: fill empty look-back slots with a photo taken on the same date). Planned in detail, not started — see [`docs/photo-lookback-plan.md`](../docs/photo-lookback-plan.md). Touches the entry-date encoding rules (photo dates must be matched in the LOCAL calendar, not UTC) and reinstates photo-library permission, so read both that plan and the two rule sections above before starting.
