...
- **Platform:** iOS 18+ (deployment target `18.0`). Lowered from the original "iOS 26+ only" before the first App Store submission — iOS 26 excluded almost every real device for no benefit, since nothing in the code needs it. The true floor is **iOS 17.0** (SwiftData `@Model`/`@Query`, `@Observable`, and `AVAudioApplication` are all 17.0); we sit at 18.0 to skip the early 17.x releases where SwiftData was rough. Use modern SwiftUI APIs freely up to iOS 18; do not add back-compatibility shims for older iOS. If you reach for an iOS 26-only API (Liquid Glass etc.), raise this floor deliberately rather than silently.
...
- **Normalisation time zone — SUPERSEDED, see "Entry date encoding" below.** ~~`Entry.date` is normalised to start-of-day in the user's current *local* calendar/time zone (`Calendar.current`), deliberately not UTC. Known limitation: if the user changes time zones after writing, the stored instant doesn't re-normalise (acceptable for a personal journal).~~ That "acceptable limitation" turned out to be a data-loss-shaped bug — entries written abroad became unreachable by every query — and was replaced in July 2026.
...
- **Permissions — REWRITTEN after App Store rejection (August 2026). See "Permission requests" below.** `Services/MediaPermissions.swift` now wraps only Camera (`AVCaptureDevice`) and Microphone (`AVAudioApplication`, the iOS-17+ API). It ONLY requests/reads authorization — never stores or transmits media (honours the privacy promise). Usage strings live in build settings as `INFOPLIST_KEY_NS*UsageDescription`. The onboarding screen no longer requests anything: it is purely informational (copy + privacy-policy link + Continue).
- **Privacy policy:** `Onboarding/PrivacyPolicyView.swift` is in-app and scrollable, written to match actual behaviour; keep it in sync if data handling ever changes. **Effective 7 September 2026** (was 19 June 2026; the photo look-back section was added). Contact address in the policy is **hello@keepsakejournal.app**. The App Store also needs a **publicly hosted copy** — the iOS policy lives at **https://www.keepsakejournal.app/privacy-ios** (`privacy-ios.html` in the `memory-journal-website` repo); `/privacy` is the DESKTOP app's separate policy, don't confuse them. Support URL: keepsakejournal.app/support. The old `memoryjournalapp.com` domain was replaced when the app was renamed to keepsake. **The two iOS copies must match word-for-word** — `scripts/compare-privacy-policy.py` diffs them and is the check to run before submitting.
...
- **Permissions:** rows show **real** status via `MediaPermissions.status(of:)`; not-determined → in-app system prompt; granted/denied → deep-link to the app's page in the Settings app (`UIApplication.openSettingsURLString`). Honest, neutral wording (On / Off / **Not set** — "Enable" was removed, see "Permission requests" below). Two rows only: Camera and Microphone. (iOS only prompts once, so already-decided permissions must be changed in Settings.)
...
### Phase 7 decisions (recorded) — photo look-back

Full plan and build log: [`docs/photo-lookback-plan.md`](../docs/photo-lookback-plan.md).

- **What it is:** where a look-back slot has no entry, show a photo the user took on that date. **Opt-in**, off by default. Today's slot is never filled with a photo — it keeps the "Create your memory" prompt.
- **Photo dates are matched in the LOCAL calendar, never the journal's UTC one.** `Entry.date` is a canonical UTC midnight; `PHAsset.creationDate` is a bare instant. `Date.localDayBounds(in:)` (Shared/JournalDay.swift) decodes the canonical day and rebuilds it as a half-open local range — the exact inverse of `journalDay(in:)`. Handing a canonical instant straight to a Photos predicate puts the window on the previous afternoon west of UTC: the same shape as the travel bug, pointing outwards.
- **Accepted limitation:** photos are matched against the user's *current* time zone, not the capture zone, so a photo taken at 00:30 in Tokyo reads as the previous day back in London and keepsake disagrees with the Photos app. True capture-local time means EXIF `DateTimeOriginal` per asset — far too expensive for a screen that renders on every launch.
- **The pick is deterministic, seeded by the target date** (`PhotoLookback.select`). Not `randomElement()`: the Journal screen re-renders constantly and the photo would visibly swap mid-scroll. Not `Date.hashValue` either — Swift seeds hashing per process, so it would change on every relaunch. Candidates are sorted by identifier first, so the answer can't depend on Photos' fetch order.
- **Ranking:** screenshots dropped, dismissed photos dropped, favourites preferred, then the seeded pick. Stills only — a video's poster frame needs a different fetch and presentation.
- **`PermissionStatus` has a `.limited` case** because "Selected Photos" fails in a way that resembles neither yes nor no: the search finds nothing on almost every date and renders as "you took no photos that day". It gets its own row rather than silence.
- **Dismissals** (`PhotoDismissals`, App/Preferences.swift) store Photos' local identifiers only — no image data, nothing meaningful off this device. Cleared by "Forget Hidden Photos" and by "Delete All Data".
- **Nothing is ever copied.** `PhotoLibraryImage` renders straight from the library via `PHImageManager`. `isNetworkAccessAllowed = true` so iCloud-only originals aren't blank — that is iOS fetching the user's own photo from their own iCloud, not a network call of ours.
- **`onAppear` is NOT a usable "screen shown again" signal in this app.** `RootTabView` keeps every tab's view alive in a ZStack, so it fires once at launch and never on tab switch. `JournalView` re-reads the permission on every lookup and keys its `.task` on the selected tab and scene phase; a cached status here is what made photos silently never appear. Three separate bugs in this phase traced back to that.
- **Toggle rows must be a `Button` over the whole row**, with the switch drawn but `allowsHitTesting(false)`. A bare `Toggle` only responds on its ~51×31pt switch, so most of a full-width row is dead space — the reason the photo and App Lock toggles felt intermittent. `accessibilityRepresentation` restores a real switch for VoiceOver; `accessibilityElement(children: .combine)` must NOT be used on these rows, as it flattens away the switch's own action.

### Permission requests (August 2026 — after an App Store rejection)

**Build 1.0 (3) was rejected under guideline 5.1.1(iv) — Legal · Privacy · Data Collection and Storage.** The onboarding "access your media" screen showed a custom message, then three **"Enable Camera / Photo Library / Microphone"** buttons plus a **"Maybe later"** escape. Apple objected to two things: persuasive button wording before a system prompt ("use words like *Continue* or *Next* instead"), and the fact that the user could dismiss the message without ever reaching the actual prompt ("the user should always proceed to the permission request after the message").

**The rules now — do not undo these:**

- **Never build a pre-permission priming screen.** No custom message that precedes a system prompt, anywhere. If a screen explains a permission and then triggers one, it violates 5.1.1(iv) regardless of wording.
- **Request in context, at the point of use, with no custom message in front.** `ComposerView.tapCamera()` and the voice-note flow already do this correctly: check status → prompt if not determined → on denial show an alert with a Settings deep link. That post-denial alert is fine (Apple's own rejection letter recommends it); a *pre*-request message is not.
- **Never use persuasive words on anything leading to a prompt** — no "Enable", "Allow", "Turn on". Settings rows say **"Not set"**, not "Enable".
- **The app must not request photo-library access at all — SUPERSEDED (Phase 7, September 2026).** ~~`PhotosPicker` runs out-of-process… `MediaCapability` has no `photoLibrary` case…~~ Still true for **attaching**: `PhotosPicker` remains the only way to add a photo to an entry, it runs out-of-process, and it needs no authorization. But photo look-back searches the library **by capture date**, which no out-of-process picker can do, so `MediaCapability.photoLibrary`, `PHPhotoLibrary` `.readWrite`, and `INFOPLIST_KEY_NSPhotoLibraryUsageDescription` are all back — this time for access the app genuinely uses, which is what 5.1.1(iv) actually asks for. `.readWrite` is requested because the Photos framework has no read-only level; the app never writes.
- **`Onboarding/MediaPermissionsView.swift` is informational only.** Heading, body copy (which keeps the verbatim privacy promise), privacy-policy link, Continue. It calls no permission API. The heading changed from "access your media" to **"photos and voice notes"** because the screen no longer asks for access.

**Where the photo-library prompt is raised (Phase 7).** Exactly one place: `LookbackPhotoOfferRow`, the card in the first empty look-back slot on the Journal screen. The onboarding toggle and the Settings toggle store **intent only** and call no Photos API — they are preferences, not primers. Both those screens say so in their file headers. Do not move, copy, or "helpfully" duplicate the request anywhere else, and never put a screen in front of it that argues for the feature.

### Entry date encoding (July 2026 — replaces the Phase 1 normalisation rule)

- **The bug it fixes:** `Entry.date` used to store midnight *in the time zone the user was in when writing*. Every read path compares dates with `==`, so changing time zone made those instants stop matching and the entry vanished from Home, Calendar, and the look-back — while still sitting in the database. Found via an entry written in Santorini that disappeared on return to the UK. DST was never a factor (`startOfDay` applies the offset in effect on that date); only zone changes.
- **The rule now:** `Entry.date` is **UTC midnight of the day the entry belongs to**. The **local** calendar still decides *which* wall-clock day that is — the day the user lived — and UTC only decides how it's *encoded*. Those are two separate steps; collapsing them into `Calendar.journal.startOfDay(for:)` files late-evening entries a day late west of UTC.
- **`Shared/JournalDay.swift` is the single funnel point:** `Calendar.journal` (a copy of `Calendar.current` pinned to UTC, so the Calendar grid keeps the locale's first weekday), `Date.journalToday`, `journalDay(in:)`, `isCanonicalJournalDay`. **Never use `Calendar.current` to build, compare, or format an entry date.** `Calendar.current` is still correct for "what day is it right now?" — which is why `DailyPrompts` rotation still flips at local midnight.
- **Two traps, both covered by tests in `JournalDayTests`:** (1) double-encoding — passing an already-canonical date into `Entry.init` re-reads it in the local zone, so `SampleData` and `ComposerView` must pass `calendar: .journal`; (2) **display** — `DateFormatter`s must be pinned to `.gmt`, or every date label renders a day early west of UTC.
- **`Services/EntryDateMigration.swift`** repairs legacy rows once, on launch, guarded by `PreferenceKey.entryDateStorageVersion`. Because the legacy value is `utcMidnight − offset` and UTC midnights are multiples of 86400, the remainder recovers the write zone's offset exactly: the answer is the UTC midnight just below or just above, whichever implies a real offset (−12h…+14h). Exact for every zone including half/quarter-hour ones; **ambiguous only at UTC+12 and further east** (+12 is indistinguishable from −12), where it flags `ambiguousCount` rather than silently guessing. Idempotent (canonical dates have remainder 0), and writes a before/after audit JSON into Application Support so any mistake is reversible. On-device only — privacy promise unaffected.
- **Testing note:** the pre-existing time-zone tests all used ONE calendar for both writing and reading, which is why they couldn't catch this. New tests deliberately **write in one zone and read in another**.
...
- [x] **Phase 7 — Photo look-back** (opt-in: fill empty look-back slots with a photo taken on the same date). See [`docs/photo-lookback-plan.md`](../docs/photo-lookback-plan.md) and "Phase 7 decisions" above. Touches the entry-date encoding rules (photo dates are matched in the LOCAL calendar, not UTC) and reinstates photo-library permission.
